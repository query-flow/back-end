...
def register(p: RegisterRequest, db: Session = Depends(get_db)):
    ...
    Nota: Este é o ÚNICO endpoint público. Todos os outros requerem autenticação JWT.
    """
    ...
    existing_org = db.query(Org).filter(Org.name == p.org_name).first()
    if existing_org:
        raise HTTPException(
            status_code=400,
...
def login(p: LoginRequest, db: Session = Depends(get_db)):
    ...
    Nota: Usuários com status='invited' não podem fazer login até aceitarem o convite.
    """
...
def accept_invite(p: AcceptInviteRequest, db: Session = Depends(get_db)):
    ...
    Nota: Token expira em 7 dias após criação.
    """
    # Buscar usuário pelo invite_token
    user = db.query(User).filter(User.invite_token == p.invite_token).first()
    if not user:
        raise HTTPException(
            status_code=404,
    ...
    org_link = db.query(OrgMember).filter(OrgMember.user_id == user.id).first()
    if not org_link:
        raise HTTPException(
            status_code=500,
    ...
    org = db.query(Org).filter(Org.id == org_link.org_id).first()
...
def refresh_access_token(p: RefreshTokenRequest):
    ...
    Nota: Refresh tokens expiram em 30 dias.
    """
...
def admin_login(p: LoginRequest, db: Session = Depends(get_db)):
    ...
    Similar ao /auth/login, mas valida que o usuário tem role='admin'.
    Platform Admins têm acesso aos endpoints /admin/* para gerenciar a plataforma.
